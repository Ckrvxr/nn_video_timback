...
def check_run_signals(run_dir: Path, exit_flag_ref: list) -> bool:
    ...
    if exit_file.exists():
        console.warning(
            f"Exit signal detected (.exit file found in {run_dir}). Stopping gracefully..."
        )
    ...
    while pause_file.exists() and not exit_flag_ref[0]:
        ...
        if first_pause_msg:
            console.warning(
                f"Training paused (.pause file found in {run_dir}). Delete the file to resume."
            )
            first_pause_msg = False
        time.sleep(1.0)
        if exit_file.exists():
            console.warning(
                f"Exit signal detected during pause (.exit file found in {run_dir}). "
                "Stopping gracefully..."
            )
            exit_flag_ref[0] = True
            try:
