...
def _plan_segments(video_path: Path, args) -> list[dict] | None:
    ...
    lr_gx = max(1, (w // sf) // patch_size)
    lr_gy = max(1, (h // sf) // patch_size)
    ...
    for seg_idx, start_frame in enumerate(starts):
        ...
        gyi = seg_rng.randint(0, max(1, lr_gy) - 1)
        gxi = seg_rng.randint(0, max(1, lr_gx) - 1)
        segments.append({
...
def process_segment(video_path: Path, seg_idx: int, start_frame: int,
    ...
    sf = args.scale_factor
    if sf > 1 and lr_scale:
        # HR: scale source down → crop (同参数与 LR 对齐)
        hr_vf = f'{lr_scale},crop={patch_size}:{patch_size}:{cx // sf}:{cy // sf}'
        # LR: 有损流已 scale 过，只需 crop
    ...
    # Determine HR pixel format from source bit depth
    hr_pix_fmt = f'yuv444p{source_bits}le' if source_bits > 8 else 'yuv444p'
