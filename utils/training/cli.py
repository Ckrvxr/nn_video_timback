...
def check_memory(config, force: bool = False):
    """Check memory using the new MemoryManager"""
    from utils.memory import get_memory_manager
    
    mem_cfg = config.get('memory_settings', {})
    max_ram_ratio = mem_cfg.get('max_ram_ratio', 0.3)
    max_vram_ratio = mem_cfg.get('max_vram_ratio', 0.5)
    
    manager = get_memory_manager(max_ram_ratio, max_vram_ratio)
    ...
    if ram_pressure or vram_pressure or force:
        ...
        if vram_pressure:
            console.warning(f"VRAM pressure detected (limit: {max_vram_ratio*100:.0f}%)")
