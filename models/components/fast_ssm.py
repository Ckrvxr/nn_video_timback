...
class FastSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, state: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Stabilize transition matrix A by scaling its infinity norm to at most 0.99.
        # This bounds the eigenvalues of A and ensures the SSM remains stable over time.
        max_norm = 0.99
        ...
        if HAS_TRITON and x.is_cuda:
            ...
            WsT = W_s.T
            for t in range(L_sz):
                out[:, t] = hx[:, t] + curr_state @ WsT
                curr_state = curr_state @ AT + u[:, t]
