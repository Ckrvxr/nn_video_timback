...
def main():
    ...
    baseline = compute_baseline(config, val_clips, logging_cfg, run_dir, output_dir) if val_clips else {}
        ...
        for epoch in range(start_epoch, n_epochs):
            ...
            if check_run_signals(run_dir, exit_flag_ref):
                ...
                save_epoch_checkpoint(
                    max(0, epoch - 1), model.state_dict(), optimizer.state_dict(),
                    0.0, {}, 0.0, 0.0, loss_weights, run_dir, output_dir,
                )
                break
            ...
            if epoch % logging_cfg.get('validation_interval', 1) == 0:
                ...
                for name, clips in val_clips.items():
                    ...
                    try:
                        psnr, ssim, vmaf, vgg = validate(model, device, clips, val_batch_size, name)
            ...
            save_epoch_checkpoint(
                epoch, model.state_dict(), optimizer.state_dict(),
                train_loss, metrics, score, avg_lr,
                loss_weights, run_dir, output_dir,
            )
    finally:
        stop_io_worker()
